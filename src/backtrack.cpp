...
Backtrack::~Backtrack()
{
	...
	if( mapping != NULL ) {
		delete[] mapping; //global_memory.returnLLArray(mapping, n2);
		mapping = NULL;
	}
...
bool Backtrack::run(Coloring* aColoring, Graph* aG1, Graph* aG2, int32_t aNumTreeNode)
{
	...
	mapping = new int32_t[n2]; //global_memory.getLLArray(n2);
	initWorkspace();
...
void Backtrack::initWorkspace()
{
	...
	heap = new Heap(n);
	weight = new int32_t[n]; //global_memory.getLLArray(n);
	numMappedParent = new int32_t[n]; //global_memory.getLLArray(n);
	candPos = new int32_t[n]; //global_memory.getLLArray(n);
	matchingOrder = new int32_t[n]; //global_memory.getLLArray(n);
	isBinary = new char[n]; // (char*)global_memory.getLLArray(n);
}
...
void Backtrack::clearWorkspace()
{
	...
	if( weight != NULL ) {
		delete[] weight; //global_memory.returnLLArray(weight, n);
		weight = NULL;
	}
	if( numMappedParent != NULL ) {
		delete[] numMappedParent; //global_memory.returnLLArray(numMappedParent, n);
		numMappedParent = NULL;
	}
	if( candPos != NULL ) {
		delete[] candPos; //global_memory.returnLLArray(candPos, n);
		candPos = NULL;
	}
	if( matchingOrder != NULL ) {
		delete[] matchingOrder; //global_memory.returnLLArray(matchingOrder, n);
		matchingOrder = NULL;
	}
	if( isBinary != NULL ) {
		delete[] isBinary; //global_memory.returnLLArray((int32_t*)isBinary, n);
		isBinary = NULL;
	}
}
...
DAG* Backtrack::buildDAG()
{
	...
	char* popped = new char[n]; //(char*)global_memory.getLLArray(n); 
	memset(popped, 0, sizeof(char) * n);
	char* visited = new char[n]; //(char*)global_memory.getLLArray(n); 
	memset(visited, 0, sizeof(char) * n);
	int32_t* bfsQueue = new int32_t[n]; //global_memory.getLLArray(n);
	...
	delete[] popped; //global_memory.returnLLArray((int32_t*)popped, n);
	delete[] visited; //global_memory.returnLLArray((int32_t*)visited, n);
	delete[] bfsQueue; //global_memory.returnLLArray(bfsQueue, n);
...
CS* Backtrack::buildCS()
{
	...
	int32_t* NC = new int32_t[e2]; //global_memory.getLLArray(e2);
	int32_t* NCSize = new int32_t[n]; //global_memory.getLLArray(n);
	for(i = 0; i < n; ++i) {
	...
	delete[] NC; //global_memory.returnLLArray(NC, e2);
	delete[] NCSize; //global_memory.returnLLArray(NCSize, n);
...
bool Backtrack::backtrack(int32_t aNumMatching)
{
	...
	int32_t* confArray = new int32_t[n]; //global_memory.getLLArray(n);
	...
	while( true ) {
		...
		if( backtrack == false ) {
			...
			if( depth == n ) { //we found an isomorphism
				delete[] confArray; //global_memory.returnLLArray(confArray, n);
				return true;
			}
...
void Backtrack::initSingle(Graph* aG1, Graph* aG2, Coloring* aColoring1, Coloring* aColoring2, int32_t aNumTreeNode)
{
	g1 = aG1;
	g2 = aG2;
	n = g1->numNode;
	...
	if(coloring == NULL)
		coloring = new Coloring(n2);

	for(int32_t i = 0; i < n; ++i) {
		coloring->numNode = n2;
		coloring->numCell = aColoring1->numCell;
		coloring->perm[2 * i] = aColoring1->perm[i];
		coloring->perm[2 * i + 1] = aColoring2->perm[i] + n;
		coloring->inv[ aColoring1->perm[i] ] = 2 * i;
		coloring->inv[ aColoring2->perm[i] + n ] = 2 * i + 1;
		coloring->color[2 * i] = 2 * aColoring1->color[i];
		coloring->color[2 * i + 1] = 2 * aColoring2->color[i];
	}
	for(int32_t i = 0; i < aColoring1->numNode; i += aColoring1->cellSize[i]) {
		coloring->cellSize[2 * i] = 2 * aColoring1->cellSize[i];
	}
...
//aColoring is a coloring of g1
void Backtrack::buildDAGSingle()
{
	int32_t* childSize = dag->childSize;
	memset(childSize, 0, sizeof(int32_t) * n);
	int32_t* parentSize = dag->parentSize;
...
//aColoring1 is coloring of g1
void Backtrack::buildCSSingle()
{
	int32_t i, j, size, prevColor, currColor;
	int32_t* color = coloring->color;
	int32_t* perm = coloring->perm;
...
//Coloring1 is the coloring of g1
int32_t Backtrack::mapBinaryCellSingle()
{
	int32_t numMatching = 0;
	int32_t i, j, u, v, child;
	int32_t numNode = coloring->numNode;
