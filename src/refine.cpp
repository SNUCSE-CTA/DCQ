...
void Refinement::initWorkspace(int32_t size)
{
	#ifdef DEBUG
	cout << __PRETTY_FUNCTION__ << endl;
	#endif
	...
	cellStack = global_memory.getLLArray(size);
	stackSize = 0;
	neighCount = global_memory.getLLArray(size);
	visitCell = global_memory.getLLArray(size);
	visitNode = global_memory.getLLArray(size);
	numVisitNode = global_memory.getLLArray(size);
	splitCell = global_memory.getLLArray(size);
	splitCount = global_memory.getLLArray(size);
	splitPos = global_memory.getLLArray(size);
}

//DEALLOCATE memory for each workspace variables
void Refinement::clearWorkspace(int32_t size)
{
	#ifdef DEBUG
	cout << __PRETTY_FUNCTION__ << endl;
	#endif
	...
	if (cellStack != NULL) {
		global_memory.returnLLArray(cellStack, size);
		cellStack = NULL;
	}
	if (neighCount != NULL) {
		global_memory.returnLLArray(neighCount, size);
		neighCount = NULL;
	}
	if (visitCell != NULL) {
		global_memory.returnLLArray(visitCell, size);
		visitCell = NULL;
	}
	if (visitNode != NULL) {
		global_memory.returnLLArray(visitNode, size);
		visitNode = NULL;
	}
	if (numVisitNode != NULL) {
		global_memory.returnLLArray(numVisitNode, size);
		numVisitNode = NULL;
	}
	if (splitCell != NULL) {
		global_memory.returnLLArray(splitCell, size);
		splitCell = NULL;
	}
	if (splitCount != NULL) {
		global_memory.returnLLArray(splitCount, size);
		splitCount = NULL;
	}
	if (splitPos != NULL) {
		global_memory.returnLLArray(splitPos, size);
		splitPos = NULL;
	}
}
...
Coloring* Refinement::runSingle(Graph* aG)
{
	n = aG->numNode;
	e = aG->numEdge;
	...
	if( nLen < n ) {
		...
		stableColoring = new Coloring(n);
		cellStack = new int32_t[n];
		neighCount = new int32_t[n];
		visitCell = new int32_t[n];
		visitNode = new int32_t[n];
		numVisitNode = new int32_t[n];
		splitCell = new int32_t[n];
		splitCount = new int32_t[n];
		splitPos = new int32_t[n];
		nLen = n;
	}
	stackSize = 0;
	stableColoring->numNode = n;
	stableColoring->numCell = 1;
	for(int32_t i = 0; i < stableColoring->numNode; ++i) {
		stableColoring->color[i] = 0;
		stableColoring->perm[i] = stableColoring->inv[i] = i;
		stableColoring->cellSize[i] = 0;
	}
	stableColoring->cellSize[0] = stableColoring->numNode;

	// Initial coloring
	colorByDegreeAndLabelSingle(stableColoring, aG);

	// Process coreness-1 vertices
	//numTreeNode = prepCoreOneSingle(stableColoring, aG);

	// Refine coloring
	refineSingle(stableColoring, aG);

	return stableColoring;
}


void Refinement::clearSingle()
{
	if(stableColoring != NULL) {
		delete stableColoring;
		stableColoring = NULL;
...
void Refinement::colorByDegreeAndLabelSingle(Coloring* coloring, Graph* aG)
{
	sort(coloring->perm, coloring->perm + coloring->numNode,
			[aG](const int32_t& i, const int32_t& j) -> bool {
				int32_t di = aG->d[i];
				int32_t li = aG->l[i];
				int32_t dj = aG->d[j];
				int32_t lj = aG->l[j];
	...
	for (int32_t i = 1; i < coloring->numNode; ++i) {
		...
		const int32_t du = aG->d[u];
		const int32_t lu = aG->l[u];

		const int32_t dv = aG->d[v];
		const int32_t lv = aG->l[v];
...
//PARTITION their neighbors so that any two vertices belong to the same cell if and only is they were previously in the same cell and they had equal number of neighbors of coreness-1
//RETURN the number of removed vertices
int32_t Refinement::prepCoreOneSingle(Coloring* aColoring, Graph* aG)
{
	...
	while( ind < stackSize ) {
		if( global_mark > INFINITY ) {
			memset(markCell, 0, sizeof(int32_t) * n * 2);
			memset(markNode, 0, sizeof(int32_t) * n * 2);
			global_mark = 0;
		}
		++global_mark;
...
//REMOVE the coreness-1 neighbors of aVertex
void Refinement::deleteEdge(int32_t aVertex, int32_t aNumDegOne, Graph* aG)
{
	int32_t n = aG->numNode;
	int32_t* d1 = aG->d;

	int32_t du = d1[aVertex];
	if( du < 2 )
		return;

	int32_t i, newDegree, temp, lastInd;
	int32_t* edge = aG->e[aVertex];
	char* one = aG->one;
...
void Refinement::refineSingle(Coloring* coloring, Graph* aG)
{
	#ifdef DEBUG
	cout << __PRETTY_FUNCTION__ << endl;
	#endif
	...
	const int32_t numNode = aG->numNode;
	

	int32_t* inStack = new int32_t[numNode]; //global_memory.getLLArray(numNode);
	int32_t* stackCand = new int32_t[numNode]; //global_memory.getLLArray(numNode);
	...
	while (stackSize > 0) {
		...
		if (global_mark > INFINITY) {
			memset(markCell, 0, sizeof(int32_t) * numNode * 2);
			memset(markNode, 0, sizeof(int32_t) * numNode * 2);
			global_mark = 0;
		}
		++global_mark;

		int32_t weightend = 0;
		int32_t idx = selectFromStackSingle(coloring);
		int32_t currCell = cellStack[idx];
		int32_t currEnd = currCell + coloring->cellSize[currCell];
...
//RETURN the index of the smallest (among the last 13 cells) cell in the stack
int32_t Refinement::selectFromStackSingle(Coloring* coloring)
{
	int32_t ret = stackSize - 1;
	int32_t idx = stackSize - 2;
	while (idx >= 0) {
		...
		if (coloring->cellSize[cellStack[ret]] == 1 || idx < stackSize - 13) {
			break;
		}
...
int32_t Refinement::getNumTreeNodeSingle()
{
	return numTreeNode;
}
